...
const ReFlv: React.FC<FlvProps> = (props: FlvProps) => {
  ...
  const CreatePlayer = useCallback(() => {
    ...
    if (Mpegts.getFeatureList().mseLivePlayback) {
      ...
      if (videoRef.current) {
        ...
        flvRef.current.on(Mpegts.Events.ERROR, (type, _detail, info) => {
  ...
  return (
    <div className={cls['flv-player-content']}>
      <video
        controls
        ref={videoRef}
        muted
      ...
      />
      <div className={cls['flv-player-content-speed']}>{curSpeed} KB/S </div>
    </div>
  );
};
