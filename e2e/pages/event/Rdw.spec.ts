...
test.describe('The Retrograde Page', () => {
  const pageUrl = '/event/retrograde';
  ...
  test('用路侧模拟器发送[车辆逆行]数据--事件列表成功接收到数据', async ({ browser }) => {
    ...
    await checkDataset(simulatorPage, 'RW_track');
    // 直到loading结束再点击发送按钮
    await simulatorPage.locator('#loading-RW_track').waitFor({ state: 'hidden' });
